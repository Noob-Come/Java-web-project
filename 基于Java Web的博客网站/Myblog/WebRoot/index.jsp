...
<div id="container">
	...
	<c:forEach var="article" items="${ requestScope.articleList }" varStatus="vs">
	    <article class="article">
	        <time>${ article.localpubDate }</time>
	        <h2 class="title"><a href="admin/article?method=gotoDetail">${ article.title }</a></h2>
	        ...
	        <footer>
	            <a href="gotoDetail?articleId=${ article.articleId }&pre=${ articleList[vs.count-1].articleId }&next=${ articleList[vs.count+1].articleId }">阅读全文</a>
	        </footer>
	    </article>
	</c:forEach>
        ...
        <div style="text-align: center">
            <ul class="pagination" >
                <li class="page-item <c:if test="${ pageData.currIndex eq 1 }">disabled</c:if> "> 
                	<a href="index?currIndex=1">&laquo;</a>
                </li>
                <c:forEach var="p" begin="1" end="${ pageData.sumPages }">
                    <li class="page-item <c:if test="${ pageData.currIndex eq p }">active</c:if> ">
                    	<a href="index?currIndex=${ p }">${ p }</a>
                    </li>
                </c:forEach>
                <li class="page-item <c:if test="${ pageData.currIndex eq pageData.sumPages }">disabled</c:if> "> 
                	<a href="index?currIndex=${ pageData.sumPages }">&raquo;</a>
                </li>
            </ul>
        </div>
</div>
