...
<script type="text/javascript">
  function checkForm(){
    var content=document.loginForm.content.value;
    var email=document.loginForm.email.value;
    var name=document.loginForm.name.value;
     if(content==""){
        document.getElementById("error").innerText="评论不能为空！";
        return false;
      }
     if(name==""){
        document.getElementById("error").innerText="昵称不能为空！";
        return false;
      }
     if(email==""){
        document.getElementById("error").innerText="邮箱不能为空！";
        return false;
     }
       return true;
  } 
...
<article class="article">
            <time id="time1">${ article.localpubDate }</time>
            ...
            <section>
                ...
                <table b>
                	<tr>
	                	<td><div><h4><span class="label label-primary">昵称</span></h4></div></td>
	                	<td><div><h4><span class="label label-success">评论内容</span></h4></div></td>
                	</tr>
                	<c:forEach var="c" items="${ requestScope.commentList }" varStatus="vs">
                	<tr color="red">
	                	<td >${ c.cNickname }:</td>
	                	<td>${ c.cContent }</td>
                	</tr>
    	...
    	<div class="form-horizontal" role="form" style="margin:10px">
    		<div class="form-group">
				...
				<div class="col-sm-3">
                      <textarea id="content" name="content" class="form-control" rows="3"  placeholder="文明上网，理性发言" ></textarea>
                      </div>
                </div>
     		   <input id="articleId" type="hidden" value="${ articleId }" >
                        ...
                        <div class="form-group" style="position:relative;left:13%">
					        ...
					        <br/>
					    	 <button id="commentButton" class="btn btn-default" type="submit" value="提交" >提交</button>
					    	    <span style="color:red;" id="info"></span>
                        </div>
    			</div>
    			<div style="position: relative;left: 12%">
		            <div ><a href="gotoDetail?articleId=${ preArticle.articleId }"><h4><span class="label label-primary">上一篇:${ preArticle.title }</span></h4></a></div>
		            <div><a href="gotoDetail?articleId=${ nextArticle.articleId }"><h4><span class="label label-success">下一篇:${ nextArticle.title }</span></h4></a></div>
		    	</div>
       </div>
 ...
 <script type="text/javascript">
 	$("#commentButton").click(function(){
 		if($("#content").val()=="") {
 			...
 			//发送异步请求
 			$.post("comment?method=addComment",
